...
def map_coordinates(I, yc, xc, Y):  # noqa
    """
...
def compute_masks(dP, cellprob, p=None, niter=200, cellprob_threshold=0.0, flow_threshold=0.4, interp=True, do_3D=False,
                  min_size=15, resize=None, use_gpu=False, device=None):
    ...
    if np.any(cp_mask):  # mask at this point is a cell cluster binary map, not labels
        ...
        if resize is not None:
            ...
            mask = utils.resize_image(mask, resize[0], resize[1], interpolation=cv2.INTER_NEAREST)
            if recast:
