...
class UnetModel():
    ...
    def __init__(
        ...
        self.net = resnet_torch.CPnet(self.nbase, self.nclasses, sz=3, residual_on=residual_on,
                                      style_on=style_on, concatenation=concatenation, mkldnn=self.mkldnn,
    ...
    def eval(self,
        ...
        """
        x = [utils.convert_image(xi, channels, channel_axis, z_axis, do_3D,
        ...
        if not do_3D:
            for i in iterator:
                ...
                img = utils.resize_image(img, rsz=rescale[i])
                ...
                if compute_masks:
                    ...
                    maski = utils.resize_image(maski, shape[-3], shape[-2],
    ...
    def _run_net(self, imgs, augment=False, tile=True, tile_overlap=0.1, bsize=224, return_conv=False):
        ...
        imgs, ysub, xsub = utils.pad_image_ND(imgs)
    ...
    def _run_tiled(self, imgi, augment=False, bsize=224, tile_overlap=0.1, return_conv=False):
        ...
        if imgi.ndim == 4:
            ...
            IMG, ysub, xsub, Ly, Lx = utils.make_tiles(imgi[0], bsize=bsize,
            ...
            if ny * nx > batch_size:
                ...
                for k in ziterator:
                    ...
                    for i in range(min(Lz - k * nimgs, nimgs)):
                        IMG, ysub, xsub, Ly, Lx = utils.make_tiles(imgi[k * nimgs + i], bsize=bsize,
                                                                   augment=augment, tile_overlap=tile_overlap)
                    ...
                    for i in range(min(Lz - k * nimgs, nimgs)):
                        ...
                        if augment:
                            ...
                            y = utils.unaugment_tiles(y, self.unet)
                            y = np.reshape(y, (-1, 3, ly, lx))
                        yfi = utils.average_tiles(y, ysub, xsub, Ly, Lx)
            ...
            IMG, ysub, xsub, Ly, Lx = utils.make_tiles(imgi, bsize=bsize,
                                                       augment=augment, tile_overlap=tile_overlap)
            ny, nx, nchan, ly, lx = IMG.shape
            ...
            yf = utils.average_tiles(y, ysub, xsub, Ly, Lx)
    ...
    def _run_3D(
        ...
        for p in range(3 - 2 * self.unet):
            ...
            # rescale image for flow computation
            shape = xsl.shape
            xsl = utils.resize_image(xsl, rsz=rescaling[p])
            # per image
            y, style = self._run_nets(xsl, net_avg=net_avg, augment=augment, tile=tile,
                                      bsize=bsize, tile_overlap=tile_overlap)
            y = utils.resize_image(y, shape[1], shape[2])
    ...
    def train(self, train_data, train_labels, train_files=None, test_data=None, test_labels=None, test_files=None,
        ...
        train_data, train_labels, test_data, test_labels, run_test = utils.reshape_train_test(
        ...
        if run_test:
            ...
            if self.nclasses == 3:
                test_classes = [
                    np.stack((label, label > 0, utils.distance_to_boundary(label)), axis=0).astype(np.float32)
    ...
    def _set_optimizer(self, learning_rate, momentum, weight_decay, SGD=False):
        if SGD:
            ...
            self.optimizer = optim.RAdam(self.net.parameters(), lr=learning_rate, betas=(0.95, 0.999),
