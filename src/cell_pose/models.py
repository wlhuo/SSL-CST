...
class Cellpose():
    ...
    def __init__(self, gpu=False, model_type='cyto', net_avg=False, device=None):
        ...
        self.pretrained_size = self.size_model_path(model_type)
...
class CellposeModel(UnetModel):
    ...
    def eval(self, x, batch_size=8, channels=None, channel_axis=None,
        ...
        if isinstance(x, list) or x.squeeze().ndim == 5:
            ...
            for i in iterator:
                channel = channels[i] if (len(channels) == len(x) and (isinstance(channels[i], list) or isinstance(
            ...
            x = utils.convert_image(x, channels, channel_axis=channel_axis, z_axis=z_axis,
    ...
    def _run_cp(self, x, compute_masks=True, normalize=True, invert=False,
        ...
        if do_3D:
            ...
            if normalize or invert:
                img = utils.normalize_img(img, invert=invert)
            ...
            for i in iterator:
                ...
                if normalize or invert:
                    img = utils.normalize_img(img, invert=invert)
                if rescale != 1.0:
                    img = utils.resize_image(img, rsz=rescale)
                yf, style = self._run_nets(img, net_avg=net_avg,
                                           augment=augment, tile=tile,
                                           tile_overlap=tile_overlap)
                if resample:
                    yf = utils.resize_image(yf, shape[1], shape[2])
...
class SizeModel():
    ...
    def eval(self, x, channels=None, channel_axis=None,
        ...
        if isinstance(x, list):
            ...
            for i in iterator:
                channel = channels
                if (channels is not None and len(channels) == len(x) and len(channels[i]) == 2 and (
                        isinstance(channels[i], list) or isinstance(channels[i], np.ndarray))):
                    channel = channels[i]
